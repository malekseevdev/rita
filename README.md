...
3.  When you have a non-trivial feature to plan, invoke the
    **`rita-plan`** skill in Claude Code:
    ...
    docs against the code for inconsistencies, gaps, open questions,
    and stalled status — proposing a code-side and a doc-side fix for
    each divergence so you choose which is the source of truth.
